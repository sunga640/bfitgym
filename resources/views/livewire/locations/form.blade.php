    ...
    <form wire:submit.prevent="save" class="mx-auto max-w-2xl space-y-8">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
            <div class="border-b border-zinc-200 pb-4 dark:border-zinc-700">
                <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">{{ __('Location Information') }}</h2>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{{ __('Enter the basic details for this location.') }}</p>
            </div>
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    <flux:field>
                        ...
                        <flux:input type="text" wire:model.live="name" placeholder="{{ __('e.g., Main Gym Floor, Studio A, Weight Room') }}" required />
                        @error('name')
                            <div class="mt-1 text-xs text-red-500">{{ $message }}</div>
                        @enderror
                    </flux:field>
                ...
                <div class="sm:col-span-2">
                    <flux:field>
                        ...
                        <flux:textarea wire:model.live="description" rows="3" placeholder="{{ __('Describe the location, its purpose, and any relevant details...') }}" />
                        @error('description')
                            <div class="mt-1 text-xs text-red-500">{{ $message }}</div>
                        @enderror
                    </flux:field>
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="flex items-center justify-between rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                        <div>
                            <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ __('Active Status') }}</p>
                            <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Inactive locations will not appear in class scheduling or equipment allocation options.') }}</p>
                        </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <flux:button variant="ghost" href="{{ route('locations.index') }}" wire:navigate>
                {{ __('Cancel') }}
            </flux:button>
            <flux:button variant="primary" type="submit" wire:loading.attr="disabled">
                <span wire:loading.remove>{{ $is_editing ? __('Update Location') : __('Create Location') }}</span>
