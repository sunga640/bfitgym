        ...
        <div class="mx-auto max-w-2xl">
            <div class="rounded-xl border border-amber-200 bg-amber-50 p-8 text-center dark:border-amber-800 dark:bg-amber-900/20">
                ...
                <p class="mt-2 text-sm text-amber-700 dark:text-amber-300">
                    {{ __('Please select a branch from the sidebar before creating an expense category.') }}
                </p>
                <div class="mt-6 flex items-center justify-center gap-3">
                    <flux:button variant="ghost" href="{{ route('expense-categories.index') }}" wire:navigate>
                        {{ __('Go Back') }}
                    </flux:button>
            ...
            <div class="rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-800">
                <div class="border-b border-zinc-200 px-6 py-4 dark:border-zinc-700">
                    <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">
                        {{ $is_editing ? __('Edit Expense Category') : __('Create Expense Category') }}
                    </h2>
                    <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        {{ __('Use categories to group expenses for better reporting.') }}
                    </p>
                </div>

                <div class="space-y-6 p-6">
                    <flux:field>
                        <flux:label required>{{ __('Category Name') }}</flux:label>
                        <flux:input
                            wire:model.live="name"
                            type="text"
                            placeholder="{{ __('e.g., Utilities, Maintenance, Staff Welfare') }}"
                            required
                        />
                    ...
                    <flux:field>
                        ...
                        <flux:textarea
                            ...
                            rows="3"
                            placeholder="{{ __('Optional notes for this category') }}"
                        />
